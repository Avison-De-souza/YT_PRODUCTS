document.addEventListener("DOMContentLoaded", async () => {
  const grid = document.getElementById('feastablesProductGrid');
  if (!grid) return;

  try {
    const response = await fetch('http://localhost:5000/api/products/feastables');
    const products = await response.json();

    grid.innerHTML = '';

    if (products.length === 0) {
      grid.innerHTML = '<p style="text-align:center;">No Feastables products found in database.</p>';
      return;
    }
    ...
    products.forEach(item => {
      ...
      const destinationUrl = item.buy_url || 'https://feastables.com';
    ...
    console.error('Error fetching Feastables products:', err);
